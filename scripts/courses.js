...
const COURSE_FILTERS = ['All', 'CSE', 'WDD'];

function removeFilterSelected(dataFilter = 'All') {
    COURSE_FILTERS
        .filter((filter) => filter != dataFilter)
        .forEach((filter) =>
            document
...
function renderCourses(filter = 'All') {
    ...
    filtered.forEach(course => {
        ...
        card.innerHTML = `
            ...
            ${course.completed ? '<span class="text-green"><strong>Completed</strong></span>' : '<span class="text-darkred"><strong>Pending</strong></span>'}`;
...
filterButtons.forEach(button => {
    button.addEventListener('click', () => {
        removeFilterSelected(button.dataset.filter);
